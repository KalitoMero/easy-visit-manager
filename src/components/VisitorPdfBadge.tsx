...
const VisitorPdfBadge = ({ visitor, onPdfGenerated }: VisitorPdfBadgeProps) => {
  ...
  // Safer initialization check that won't block rendering
  useEffect(() => {
    const checkPdfLibrary = () => {
      try {
        const isReady = isPdfMakeInitialized();
        setIsPdfLibraryReady(isReady);
        logDebug('PDF', `PDF library initialization status: ${isReady ? 'Ready' : 'Not ready'}`);
      } catch (e) {
        logDebug('PDF', 'Error checking PDF library status:', e);
        setIsPdfLibraryReady(false);
      }
    };
    ...
    // Retry after a delay to account for async loading
  ...
  try {
          ...
          <div className="flex flex-col gap-2">
            {!isPdfLibraryReady && (
              <Alert variant="warning" className="mb-2">
                ...
                <AlertTitle>PDF-Library wird geladen</AlertTitle>
                <AlertDescription>
                  Die PDF-Bibliothek wird geladen. Bitte warten Sie einen Moment...
                </AlertDescription>
              </Alert>
            )}
    ...
    logDebug('PDF', 'Error rendering VisitorPdfBadge component:', e);
    return (
      <Alert variant="destructive">
        <AlertTriangle className="h-4 w-4" />
        <AlertTitle>Rendering-Fehler</AlertTitle>
        <AlertDescription>
          Bei der Anzeige der PDF-Komponente ist ein Fehler aufgetreten.
        </AlertDescription>
      </Alert>
    );
  }
